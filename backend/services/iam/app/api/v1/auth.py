...
from ...utils import microsoft_oauth
from ...utils import apple_oauth
import random
import string
import uuid
...
async def signup(user_data: UserCreate, response: Response, db: AsyncSession = Depends(get_db)):
    ...
    # Create tokens
    access_token = create_token(data={"sub": str(new_user.id), "email": new_user.email}, secret_key=settings.SECRET_KEY, algorithm=settings.ALGORITHM)
    refresh_token = create_token(data={"sub": str(new_user.id)}, secret_key=settings.SECRET_KEY, algorithm=settings.ALGORITHM, token_type="refresh")
...
async def login(credentials: UserLogin, response: Response, db: AsyncSession = Depends(get_db)):
    ...
    access_token = create_token(data={"sub": str(user.id), "email": user.email}, secret_key=settings.SECRET_KEY, algorithm=settings.ALGORITHM, expires_delta=access_token_expires)
    refresh_token = create_token(data={"sub": str(user.id)}, secret_key=settings.SECRET_KEY, algorithm=settings.ALGORITHM, expires_delta=refresh_token_expires, token_type="refresh")
...
async def refresh_token(request: Request, response: Response):
    ...
    try:
        ...
        # Create new tokens
        access_token = create_token(data={"sub": user_id, "email": email}, secret_key=settings.SECRET_KEY, algorithm=settings.ALGORITHM)
        new_refresh_token = create_token(data={"sub": user_id}, secret_key=settings.SECRET_KEY, algorithm=settings.ALGORITHM, token_type="refresh")
...
async def get_current_user_info(
    ...
    try:
        ...
        # Decode token
        from cognitest_common import decode_token
...
async def logout(response: Response, current_user: User = Depends(get_current_active_user)):
    ...
    await SessionService.delete_session(redis_client, str(current_user.id))
...
async def get_session(current_user: User = Depends(get_current_active_user)):
    ...
    """
    redis_client = await get_redis_client(settings.REDIS_URL)
    session = await SessionService.get_session(redis_client, str(current_user.id))
    
    # Extend session TTL on activity
    await SessionService.extend_session(redis_client, str(current_user.id))
...
async def update_session(
    ...
    redis_client = await get_redis_client(settings.REDIS_URL)
    success = await SessionService.update_session(redis_client, str(current_user.id), filtered_data)
...
async def link_account(
    ...
    try:
        if provider == "google":
            from ...utils.google_oauth import get_user_info_from_id_token
            user_info = await get_user_info_from_id_token(id_token)
